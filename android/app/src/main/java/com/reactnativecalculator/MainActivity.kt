...
class MainActivity : ReactActivity(), ReactInstanceEventListener {
  ...
  fun updateUI(incomingWindowLayoutInfo: WindowLayoutInfo?, manual: Boolean) {
    ...
    fun collectAndCancel(windowLayoutInfo: WindowLayoutInfo, doJob: Boolean) {
      ...
      // windowLayoutInfo
      Log.d("LOG", "RESPONSE ${windowLayoutInfo}");
      ...
      //@Suppress("DEPRECATION")
      fun getRootWindowInsetsCompatM(rootView: View): Unit {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) getRootWindowInsetsCompatR(rootView) // 11 to 16
      else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) getRootWindowInsetsCompatM(rootView) // 6 to 10
